 ...
 */
export const generateFonts = async (inputDir, fontOutputDir) => {
    const fontResult = {
        dartRoundTypes: [],
        dartRoundList: [],
        dartSharpTypes: [],
    ...
    if (roundFonts.ttf && roundFonts.woff2) {
        ...
        throw new Error("Failed to build round fonts: " + (!roundFonts.ttf ? "ttf " : "") + !roundFonts.woff2 ? "woff2" : "");
    }
    ...
    if (sharpFonts.ttf && sharpFonts.woff2) {
        ...
        throw new Error("Failed to build sharp fonts: " + (!sharpFonts.ttf ? "ttf " : "") + !sharpFonts.woff2 ? "woff2" : "");
    }
    return fontResult;
};
/**
 ...
 */
const buildFontFile = async (type, fontResult, inputDir) => {
 ...
 */
function getDartIconDefinition(iconName, unicode, type) {
    return `  static const IconData ${iconName} = IconData(0x${unicode}, fontFamily: _family${capitalizeFirstLetter(type)}, fontPackage: _package);`;
}
/**
 ...
 */
function getDartIconListItem(iconName) {
